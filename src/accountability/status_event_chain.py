...
(ordered by the actual UTC instant of ``created_at`` then ``id``), following
the same rules as the authorization decision event chain:
...
* ``content_hash`` = SHA-256 of the compact, key-sorted JSON document built
  ...
  from_status, to_status, created_at}`` — the record identifier, its
  machine/event/incident ownership, the status edge, and the creation time.
...
* ``previous_status_event_id`` is ``None`` for a machine's first record and
  the immediately preceding record's id otherwise.

A successful incident status transition updates the incident row and appends
the history record to the machine's chain tail inside a single locked write
transaction, so concurrent transitions cannot lose records, fork the chain,
or break a link, and the status, history, and chain fields commit together
or not at all.
"""
...
def backfill_chains(engine: Engine) -> None:
    ...
    concurrent transition can neither interleave with the backfill nor fork.
    """
...
def append_status_event(
    ...
    *,
    ...
    """Insert one history record linked to the machine's chain tail.

    Must run inside the locked write transaction of the incident status
    transition, so the incident update, the history append, and the chain
    link commit together or not at all. The record id and timestamp are
    minted here and are guaranteed to sort strictly after the current tail in
    ``(created_at, id)`` order, so the previous-record link always matches
    the order used by backfill and verification even under same-timestamp
    concurrency. Returns the stored history record.
    """
    rows = _load_records(conn, machine_id)
    ...
    if tail is not None:
        ...
        while created_at == tail_created_at and status_event_id <= tail_id:
            status_event_id = str(uuid.uuid4())
...
def verify_machine_chain(
    ...
    Returns ``(valid, checked_count, broken_status_event_id)``. Records are
    examined in the order of the actual UTC instant of ``created_at`` and
    then ``id``. The first record's previous-status-event id must be empty;
    every later record's must be the id of the immediately preceding record,
    and each record's stored content and chain digests must match the digests
    recomputed under the creation-time rules. The first mismatching record is
    reported by its stored id verbatim; an empty chain is valid.

    The check is tolerant of damaged stored values: a corrupted
    ``created_at``, id, digest, or reference never crashes the query, is
    never repaired or recomputed for storage, and never removes the record
    from the total. Only rows whose stored ``machine_id`` equals the path
