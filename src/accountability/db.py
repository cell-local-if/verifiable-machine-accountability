...
class MachineStatusEvent(Base):
    ...
    Rows are append-only: each is written in the same locked write transaction
    as the machine status change it records, and is never updated or deleted
    afterwards, so the table is a complete, strictly machine-isolated history
    of every accepted status change.
    """
