...
A successful assignment is appended to the machine's chain tail inside a
single locked write transaction, so concurrent creators cannot lose records,
fork the chain, or break a link.
"""
...
def append_assignment(
    ...
    *,
    ...
    The ownership lookups, the duplicate check, and the insert all happen
    inside one locked write transaction, so concurrent creators cannot lose
    records, fork the chain, or break a link. The record id and timestamp are
    minted inside the write lock and are guaranteed to sort after the current
    tail in (created_at, id) order, so the previous-assignment link always
    matches the order used by backfill and verification even under
    same-timestamp concurrency. Returns a status dict: ``not_found``,
    ``duplicate_assignment``, or ``ok`` with the new ``assignment`` record.
    """
