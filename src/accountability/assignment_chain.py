...
Each incident's responsibility assignments form an ordered chain (ordered by
...
* ``chain_hash`` = SHA-256 of ``<previous chain_hash>:<content_hash>``; the
  first record of an incident uses the empty string as the previous chain
  hash.
* ``previous_assignment_id`` is ``None`` for an incident's first record and
  the prior record's id otherwise.

New assignments are appended to their incident's chain tail inside a single
write transaction, so concurrent appends cannot lose records, fork the chain,
or break a link — and never modify incidents, events, evidence, other chains,
or causal links.
"""
...
def backfill_chains(engine: Engine) -> None:
    ...
    Processing is per incident in (created_at, id) order. The recomputation is
    deterministic, so a restart over an already complete database issues no
    writes. Each incident is handled inside a locked transaction so a
...
def append_assignment(
    ...
    *,
    ...
    """Atomically append one assignment to its incident's chain tail.

    The ownership lookups, the duplicate ``(party, role)`` check, and the
    chain append all happen inside one locked write transaction, so concurrent
    appends cannot lose records, fork the per-incident chain, or both insert
    the same pair. Returns a status dict:

    * ``not_found`` — the machine, event, or incident is missing or owned by
      another machine/event;
    * ``duplicate_assignment`` — the same ``(party, role)`` pair already
      exists on this incident (nothing is written);
    * ``ok`` — with the new ``assignment`` record.
    """
    ...
    def _work(conn: Connection) -> dict[str, Any]:
        ...
        if any(
            ...
            for values in _recompute_rows(rows):
                assignment_id = values.pop("id")
                conn.execute(
                    _TABLE.update().where(_TABLE.c.id == assignment_id).values(**values)
                )
            rows = _load_assignments(conn, incident_id)
...
def verify_chain(
    session, machine_id: str, event_id: str, incident_id: str
    ...
    differs from the stored values is reported; an empty chain is valid. Only
    records owned by the path machine, event, and incident are examined.
    """
    rows = list(
        session.execute(
            _TABLE.select()
