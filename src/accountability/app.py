...
def create_responsibility_assignment(
    ...
    nothing. Path validation, the duplicate check, and the chain append happen
    in one locked write transaction, so concurrent assignments cannot lose
    records, fork, or break the per-incident chain. The transaction only ever
    writes the responsibility-assignments table: the incident, event,
    evidence, the other hash chains, and causal links are never modified.
    """
    engine = session.get_bind()
    # Release the read connection before opening the locked write transaction,
    ...
    session.close()
    result = assignment_chain.assign_responsibility(
        engine,
        machine_id=machine_id,
        event_id=event_id,
        incident_id=incident_id,
...
@app.get(
    "/machines/{machine_id}/authorization-decision-events/{event_id}/incidents/"
    "{incident_id}/responsibility-assignments/integrity",
    response_model=ResponsibilityAssignmentIntegrityOut,
)
def check_responsibility_assignment_integrity(
    machine_id: str,
    event_id: str,
    incident_id: str,
    session: SessionDep,
):
    """Read-only verification of one incident's responsibility-assignment chain.

    The machine, event, and incident must all exist and belong together; a
    missing one or an ownership mismatch returns 404 ``not_found``. Otherwise
    the incident's assignments are checked in ``(created_at, id)`` order and
    the endpoint returns ``{valid, checked_count, broken_assignment_id}``: an
    empty or fully sound chain reports ``true``, the incident's total
    assignment count, and ``null``; otherwise the first assignment whose
    content hash, previous-assignment link, or chain hash does not verify is
    reported. Only the path incident's assignments are examined, so damage
    under another incident or machine never fails this audit. The query never
    writes, repairs, or deletes and gives identical results on repeat calls
    and across restarts.
    """
    incident = incidents.get_machine_event_incident(
        session, machine_id, event_id, incident_id
