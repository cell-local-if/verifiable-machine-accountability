...
class IncidentStatusEventComplianceExportOut(BaseModel):
    machine_id: str
    from_created_at: str
    to_created_at: str
...
@app.get(
    ...
    response_model=IncidentStatusEventComplianceExportOut,
)
def export_incident_status_events_compliance(
    machine_id: str,
    params: Annotated[ComplianceExportParams, Depends(validate_compliance_export_params)],
    session: SessionDep,
):
    """Read-only compliance export of one machine's incident status history
    over a time window.

    Includes every status-history record owned by the path machine whose
    ``created_at`` falls within the inclusive bounds, in ``(created_at, id)``
    order, with exactly the fields of the status-history list endpoint.
    Records are exported exactly as stored: a missing, foreign, or damaged
    incident or event reference, or a corrupted status edge, never causes a
    record to be rewritten, filtered out, or repaired. The endpoint only
    issues reads and never returns another machine's history.
    """
    machine = session.get(Machine, machine_id)
    if machine is None:
        return error_response(404, "not_found")
    ...
    machine_history = session.scalars(
        ...
        .order_by(
            IncidentStatusEvent.created_at,
            IncidentStatusEvent.id,
        )
    ).all()
    records = [
        record
    ...
    return IncidentStatusEventComplianceExportOut(
        machine_id=machine_id,
        from_created_at=params.from_created_at,
        to_created_at=params.to_created_at,
