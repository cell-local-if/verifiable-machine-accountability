...
def create_responsibility_assignment(
    ...
    nothing. On success the record is appended to the machine's assignment
    hash chain inside a single locked write transaction, so concurrent
    creators cannot lose records, fork the chain, or break a link. The write
    touches only the responsibility-assignments table: the incident, event,
    evidence, decision-event chain, and causal links are never modified.
    """
    engine = session.get_bind()
    # Release the read connection before opening the locked write transaction,
...
def check_responsibility_assignment_integrity(machine_id: str, session: SessionDep):
    ...
    repairs, or deletes. A missing machine returns ``404 not_found``.
    """
    machine = session.get(Machine, machine_id)
    if machine is None:
        return error_response(404, "not_found")
