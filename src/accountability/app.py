...
def export_responsibility_assignments_compliance(
    ...
    exactly the fields of the assignment list endpoint. Assignments are
    exported exactly as stored: a missing or foreign event or incident
    reference, a damaged field, or a broken chain link never causes a record
    to be rewritten, filtered out, or repaired. The endpoint only issues
    reads and never returns another machine's assignments.
    """
    machine = session.get(Machine, machine_id)
    if machine is None:
        return error_response(404, "not_found")
