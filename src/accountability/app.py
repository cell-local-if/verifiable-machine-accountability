...
def export_evidence_compliance(
    ...
    order, with exactly the fields of the evidence list items. Records are
    exported as stored: a dangling or cross-machine ``event_id`` never causes
    a record to be rewritten, filtered out, or repaired. The endpoint only
    issues reads: it never writes, repairs, or deletes records, and never
    returns data owned by another machine.
    """
    machine = session.get(Machine, machine_id)
    if machine is None:
        return error_response(404, "not_found")
