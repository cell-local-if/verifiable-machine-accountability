...
def create_responsibility_assignment(
    ...
    nothing. The ownership lookups, duplicate check, and the append to the
    incident's hash-chain tail happen inside a single locked write
    transaction, so concurrent creates cannot lose records, fork, or break the
    chain; the incident, event, evidence, other hash chains, and causal links
    are never modified.
    """
    engine = session.get_bind()
    # Release the read connection before opening the locked write transaction,
    # matching the other chain appenders: concurrent creates never hold two
...
def check_responsibility_assignment_integrity(
    ...
    the incident's assignments are verified in ``created_at``, then ``id``
    order and the result is ``{valid, checked_count, broken_assignment_id}``:
    an empty or fully sound chain reports ``true``, the incident's total
    assignment count, and ``null``; otherwise the first record whose content
    hash, previous-assignment link, or chain hash does not verify is reported.
    Only the path incident's records are examined, and the query never writes,
    repairs, or deletes.
    """
    incident = incidents.get_machine_event_incident(
        session, machine_id, event_id, incident_id
