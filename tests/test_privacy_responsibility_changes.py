...
- validation before any machine or assignment record is read: ``bad_limit``
...
- GET-only ``405`` without reading records, ``404 not_found`` for a missing
  machine carrying no assignment records, and ``500 internal_error`` with no
  partial page when the records cannot be read;
- the fixed ``{machine_id, limit, records, next_cursor, has_more}``
  envelope, where each record carries exactly the ten desensitized
  privacy-responsibility export fields (the eight stored fields plus
  ``party_ref``/``role_ref``) exactly as stored, and only the path
  machine's records; raw party/role never appear;
- keyset pagination in (actual UTC created_at instant, record id) order, an
  ...
  and unlocatable-position pages;
...
def test_validation_errors_do_not_read_records(client, machine_id):
    ...
    with client.app.state.engine.begin() as conn:
        conn.execute(
            text("DROP TABLE incident_responsibility_assignments")
        )
...
def test_missing_machine_is_404_with_no_assignment_records(client):
...
def test_records_carry_exactly_the_desensitized_fields(client, machine_id):
    ...
    assert record == {
        ...
        "content_hash": HASH_A,
        "chain_hash": HASH_B,
        "party_ref": expected_ref("party", machine_id, "alice"),
        "role_ref": expected_ref("role", machine_id, "lead"),
    }
    # Raw responsibility text and its stored column names never appear.
    assert "party" not in record
    assert "role" not in record
    assert stored["party"] == "  alice "


def test_digest_strips_surrounding_whitespace(client, machine_id):
    insert_assignment(
        client, machine_id, 1, T1,
        party="  alice\t", role="\n lead  ",
    )
    (record,) = client.get(changes_url(machine_id, limit=10)).json()["records"]
    assert record["party_ref"] == expected_ref("party", machine_id, "alice")
    assert record["role_ref"] == expected_ref("role", machine_id, "lead")
...
def test_blank_party_or_role_digest_is_null_but_record_kept(client, blank):
    ...
    insert_assignment(
        client, machine_id, 1, T1, party=blank, role="lead"
    )
    insert_assignment(
        client, machine_id, 2, T2, party="alice", role=blank
    )
...
def test_non_string_party_or_role_digest_is_null_but_record_kept(
    client, machine_id
):
    # SQLite TEXT affinity would coerce a numeric literal to text, so store
    # BLOBs (which TEXT affinity leaves untouched) to get genuinely
    # non-string values. The changes view must still return the record with
    # null refs rather than coercing or leaking either value.
    insert_assignment(
        client, machine_id, 1, T1,
...
def test_raw_party_and_role_never_appear_in_response(client, machine_id):
    ...
    whole = "".join(bodies)
    assert secret_party not in whole
    assert secret_role not in whole
    assert '"party"' not in whole
    assert '"role"' not in whole


def test_digest_is_scoped_to_machine(client):
    machine_one = client.post(
        "/machines",
        json={"external_id": "machine-1", "display_name": "One",
              "public_key": "k1"},
    ).json()["id"]
    machine_two = client.post(
        "/machines",
        json={"external_id": "machine-2", "display_name": "Two",
              "public_key": "k2"},
    ).json()["id"]
    insert_assignment(client, machine_one, 1, T1, event_id=rid(101),
                      incident_id=rid(201), party="alice", role="lead")
    insert_assignment(client, machine_two, 2, T1, event_id=rid(102),
                      incident_id=rid(202), party="alice", role="lead")
...
def test_only_the_path_machines_records_are_returned(client):
    machine_one = client.post(
        "/machines",
        json={"external_id": "machine-1", "display_name": "One",
              "public_key": "k1"},
    ...
    insert_assignment(client, machine_one, 1, T0)
    insert_assignment(client, machine_two, 2, T1)
    insert_assignment(client, machine_one, 3, T2)

    body = client.get(changes_url(machine_one, limit=10)).json()
    assert [r["id"] for r in body["records"]] == [rid(1), rid(3)]
    assert all(r["machine_id"] == machine_one for r in body["records"])

    other_body = client.get(changes_url(machine_two, limit=10)).json()
...
def test_damaged_references_are_emitted_verbatim(client):
    machine_one = client.post(
        "/machines",
        json={"external_id": "machine-1", "display_name": "One",
              "public_key": "k1"},
    ...
    dangling_event_id = rid(500)
    dangling_incident_id = rid(600)
    foreign_event_id = rid(501)
    foreign_incident_id = rid(601)
    # Rows owned by machine two that machine one merely references.
    insert_assignment(
        client, machine_two, 90, T0,
        event_id=foreign_event_id, incident_id=foreign_incident_id,
    )
    insert_assignment(
        client, machine_one, 1, T1,
        event_id=dangling_event_id, incident_id=dangling_incident_id,
        previous_assignment_id=None, content_hash=HASH_A, chain_hash=HASH_B,
    )
    insert_assignment(
        client, machine_one, 2, T2,
        event_id=foreign_event_id, incident_id=foreign_incident_id,
        previous_assignment_id=rid(99),
        content_hash="z" * 64, chain_hash="0" * 64,
    )

    rows = client.get(changes_url(machine_one, limit=10)).json()["records"]
    assert [r["id"] for r in rows] == [rid(1), rid(2)]
    ...
    assert rows[1]["event_id"] == foreign_event_id
    assert rows[1]["incident_id"] == foreign_incident_id
    assert rows[1]["previous_assignment_id"] == rid(99)
    assert rows[1]["content_hash"] == "z" * 64
    assert rows[1]["chain_hash"] == "0" * 64


def test_duplicate_rows_are_both_kept(client, machine_id):
    # Two rows duplicate the same dangling incident; both stay pageable.
    insert_assignment(
        client, machine_id, 1, T1,
        event_id=rid(500), incident_id=rid(600),
        party="alice", role="lead",
    )
    insert_assignment(
        client, machine_id, 2, T2,
        event_id=rid(500), incident_id=rid(600),
        party="other-ops", role="scribe",
    )
    rows = client.get(changes_url(machine_id, limit=10)).json()["records"]
    assert [r["id"] for r in rows] == [rid(1), rid(2)]
    assert rows[0]["party_ref"] == expected_ref("party", machine_id, "alice")
    assert rows[1]["role_ref"] == expected_ref("role", machine_id, "scribe")
...
def test_new_earlier_inserts_do_not_revisit_returned_pages(client, machine_id):
    ...
    # Insert an assignment sorting before the cursor position and one
    # between.
    ...
    def table_state():
        with client.app.state.engine.connect() as conn:
            return list(
                conn.execute(
                    text(
                        "SELECT * FROM incident_responsibility_assignments"
                    )
                )
            )
...
def test_utf8_party_is_round_tripped_into_digest_byte_stably(client, machine_id):
    # The raw non-ASCII party text is never emitted, but it feeds the
    # digest; the response is compact UTF-8 JSON and byte-stable.
    insert_assignment(
        client, machine_id, 1, T1, party="  责任人-★  ", role="lead"
    )
    ...
    assert body["records"][0]["party_ref"] == expected_ref(
        "party", machine_id, "责任人-★"
    )
    assert "责任人".encode("utf-8") not in first
...
def test_real_assignments_are_pageable(client, machine_id):
    # Assignments created through the real write path keep their desensitized
    # view and walk the cursor chain end to end.
    client.post(
    ...
    assignment = client.post(
        ...
        json={"party": "alice", "role": "lead"},
    ).json()

    records, pages = fetch_all_pages(client, machine_id, limit=1)
    assert pages == 1
    (record,) = records
    assert record["id"] == assignment["id"]
