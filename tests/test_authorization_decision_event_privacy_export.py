"""Tests for the read-only desensitized privacy authorization-decision-event
export.

Covers `GET /machines/{machine_id}/authorization-decision-events/privacy-export`:
the strict query validation (``bad_time`` / ``invalid_query`` before any
machine or event read), ``404 not_found``, GET-only ``405`` routing,
closed-UTC-window filtering on each record's own ``created_at``, ordering by
the actual UTC instant then record id (exact-second before fractional-second),
the ``action_ref`` / ``resource_ref`` SHA-256 desensitizing digests (including
...
def allow_event(client, machine_id, action_type="read", resource="doc-1"):
    """Create one allowed decision event through the real write path."""
    client.post(
        "/policy-rules",
        json={
...
def insert_event_row(
    ...
    *,
    ...
    """Insert a decision-event row directly with a fixed id and timestamp."""
...
def test_event_exported_with_desensitized_fields(client):
    machine_id = create_machine(client)
    created = allow_event(client, machine_id, "read", "doc-1")
    listed = client.get(
        f"/machines/{machine_id}/authorization-decision-events"
    ).json()[0]
    ...
    assert exported["allowed"] is True
    assert exported["reason"] == "allowed_by_policy"
...
def test_blank_value_digest_is_null_but_record_kept(client, blank):
    ...
    insert_event_row(
        client, machine_id, rid(1), T1,
        action_type=blank, resource="doc-1",
    )
    insert_event_row(
...
def test_non_string_value_digest_is_null_but_record_kept(client):
    ...
    insert_event_row(
        client, machine_id, rid(1), T1,
        action_type=b"\xff\xfe binary-action", resource=b"\x00\x01 bin",
    )
    rows = client.get(export_url(machine_id)).json()["events"]
    assert len(rows) == 1
    assert rows[0]["action_ref"] is None
...
def test_raw_action_and_resource_never_appear_in_response(client):
    machine_id = create_machine(client)
    secret_action = "the-secret-action"
    secret_resource = "the-secret-resource"
    insert_event_row(
        client, machine_id, rid(1), T1,
...
def test_digest_is_scoped_to_machine(client):
    ...
    insert_event_row(
        ...
        action_type="read", resource="doc-1",
    )
    insert_event_row(
...
def test_window_is_closed_on_both_ends(client):
    machine_id = create_machine(client)
    insert_event_row(client, machine_id, rid(1), T1)
    insert_event_row(client, machine_id, rid(2), T2)
    insert_event_row(client, machine_id, rid(3), T3)

    response = client.get(export_url(machine_id, T1, T3))

    assert [e["id"] for e in response.json()["events"]] == [
        rid(1),
        rid(2),
        rid(3),
    ]
...
def test_window_excludes_events_outside_bounds(client):
    machine_id = create_machine(client)
    insert_event_row(client, machine_id, rid(10), T0)
    insert_event_row(client, machine_id, rid(11), T1)
    insert_event_row(client, machine_id, rid(12), T2)
    insert_event_row(client, machine_id, rid(13), T3)
    insert_event_row(client, machine_id, rid(14), T4)
...
def test_empty_window_returns_empty_array(client):
    ...
    insert_event_row(client, machine_id, rid(2), T4)
...
def test_fractional_second_timestamp_is_filtered_as_an_instant(client):
    machine_id = create_machine(client)
    insert_event_row(client, machine_id, rid(11), "2026-03-01T00:00:00.500000Z")
    insert_event_row(client, machine_id, rid(12), T1)

    response = client.get(export_url(machine_id, T0, T1))

    assert [e["id"] for e in response.json()["events"]] == [rid(11), rid(12)]


def test_events_ordered_by_created_at_then_id(client):
    machine_id = create_machine(client)
    # Insert out of order; two rows share T2 and must sort by id.
    insert_event_row(client, machine_id, rid(30), T3)
    insert_event_row(client, machine_id, rid(21), T2)
    insert_event_row(client, machine_id, rid(20), T2)
    insert_event_row(client, machine_id, rid(10), T1)
...
def test_same_second_exact_second_sorts_before_fractional_seconds(client):
    ...
    machine_id = create_machine(client)
    insert_event_row(client, machine_id, rid(20), "2026-03-01T00:00:00.900000Z")
    insert_event_row(client, machine_id, rid(10), "2026-03-01T00:00:00.500000Z")
    insert_event_row(client, machine_id, rid(1), "2026-03-01T00:00:00Z")
...
def test_export_never_contains_other_machine_events(client):
    ...
    insert_event_row(client, machine_two, rid(3), T2)

    response = client.get(export_url(machine_one))

    assert response.status_code == 200
    exported = response.json()["events"]
    assert [e["id"] for e in exported] == [rid(1)]
    assert all(e["machine_id"] == machine_one for e in exported)
...
def test_damaged_chain_fields_are_exported_unmodified(client, tmp_path):
    ...
    connection.execute(
        ...
        "SET previous_event_id = NULL, chain_hash = ? "
        "WHERE id = ?",
        ("f" * 64, record["id"]),
    )
    connection.commit()
...
def test_misowned_previous_event_link_is_exported_verbatim(client):
    machine_one = create_machine(client, "machine-1")
    machine_two = create_machine(client, "machine-2")
    insert_event_row(client, machine_two, rid(9), T1)
    # A record of machine one pointing at machine two's record: kept as stored.
    insert_event_row(
        client, machine_one, rid(1), T2, previous_event_id=rid(9)
    )

    (exported,) = client.get(export_url(machine_one)).json()["events"]

    assert exported["previous_event_id"] == rid(9)
...
def test_body_is_compact_newline_terminated_json_with_fixed_field_order(client):
    ...
    expected_item = {
        ...
        "resource_ref": expected_ref("resource", machine_id, "doc-1"),
        "allowed": True,
        "reason": "allowed_by_policy",
        "created_at": T1,
        "previous_event_id": None,
...
def test_export_is_read_only_and_byte_stable(client):
    ...
    allow_event(client, machine_id, "read", "doc-1")
    allow_event(client, machine_id, "read", "doc-2")
    allow_event(client, other_machine, "read", "doc-9")
...
def test_export_persists_across_restart(tmp_path, monkeypatch):
    ...
    with TestClient(app) as first:
        ...
        allow_event(first, machine_id, "read", "doc-1")
        allow_event(first, machine_id, "read", "doc-2")
