"""Tests for the read-only desensitized authorization-decision-event
privacy export.

Covers `GET /machines/{machine_id}/authorization-decision-events/
privacy-export`: the strict query validation (``bad_time`` /
``invalid_query`` before any machine or event read), ``404 not_found``,
GET-only ``405`` routing, closed-UTC-window filtering on each event's own
``created_at``, ordering by the actual UTC instant then event id
(exact-second before fractional-second), the ``action_ref`` /
``resource_ref`` SHA-256 desensitizing digests (including ``null`` for
non-string or blank values), the absence of raw action/resource text,
verbatim export of corrupted, missing, misowned, or duplicated data,
machine isolation, the fixed-field-order compact newline-terminated body,
...
def record_event(client, machine_id, action_type="read", resource="res/1"):
    response = client.post(
...
def test_event_exported_with_desensitized_fields(client):
    ...
    listed = record_event(client, machine_id, "read", "res/1")
    ...
    assert exported["reason"] == listed["reason"]
    ...
    assert exported["resource_ref"] == expected_ref("resource", machine_id, "res/1")
...
def test_blank_value_digest_is_null_but_record_kept(client, blank):
    ...
    insert_event_row(
        client, machine_id, eid(1), T1,
        action_type=blank, resource="res/1",
    )
    insert_event_row(
...
def test_digest_is_scoped_to_machine(client):
    ...
    ref_one = client.get(export_url(machine_one)).json()["events"][0]["action_ref"]
    ref_two = client.get(export_url(machine_two)).json()["events"][0]["action_ref"]
...
def test_window_is_closed_on_both_ends(client):
    ...
    assert [r["id"] for r in response.json()["events"]] == [eid(1), eid(2), eid(3)]
...
def test_duplicated_business_data_is_kept(client):
    machine_id = create_machine(client)
    # Two distinct rows carrying identical business content: both retained.
    insert_event_row(client, machine_id, eid(1), T1)
    ...
    assert [r["id"] for r in rows] == [eid(1), eid(2)]
    assert rows[0]["action_ref"] == rows[1]["action_ref"]
    assert rows[0]["resource_ref"] == rows[1]["resource_ref"]
...
def test_damaged_chain_fields_are_exported_unmodified(client, tmp_path):
    machine_id = create_machine(client)
    allow_read(client, machine_id)
    record = record_event(client, machine_id)
...
def test_misowned_previous_event_link_is_exported_verbatim(client):
    ...
    # An event of machine one pointing at machine two's event: kept as stored.
    insert_event_row(
...
def test_damaged_created_at_never_crashes_and_is_emitted_verbatim(client):
    machine_id = create_machine(client)
    insert_event_row(client, machine_id, eid(1), T1)
    insert_event_row(client, machine_id, eid(2), "not-a-timestamp")

    # A damaged stamp sorts after every parseable instant, so it never falls
    # inside a normal finite window and the good event is still returned.
    response = client.get(export_url(machine_id, WIDE[0], WIDE[1]))
    assert response.status_code == 200
    assert [r["id"] for r in response.json()["events"]] == [eid(1)]

    # A window reaching the far-future sentinel retains the damaged row
    # verbatim rather than repairing or dropping it.
    far_future = "9999-12-31T23:59:59.999999Z"
    response = client.get(export_url(machine_id, WIDE[0], far_future))
    assert response.status_code == 200
...
def test_export_is_read_only_and_byte_stable(client):
    ...
    record_event(client, machine_id, "read", "res/2")
    record_event(client, other_machine, "read", "res/other")
...
def test_empty_database_window_is_a_complete_empty_result(client):
    machine_id = create_machine(client)
    response = client.get(export_url(machine_id))
    assert response.status_code == 200
...
def test_export_persists_across_restart(tmp_path, monkeypatch):
    ...
    with TestClient(app) as first:
        ...
        record_event(first, machine_id, "read", "res/2")
