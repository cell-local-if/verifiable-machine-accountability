"""Tests for the stable, read-only incremental machine key-rotation query.

Covers `GET /machines/{machine_id}/key-rotation-events/changes`:

- validation before any machine or rotation record is read: ``bad_limit``
...
- GET-only ``405`` without reading records, ``404 not_found`` for a missing
  machine carrying no rotation records, and ``500 internal_error`` with no
  partial page when the records cannot be read;
- the fixed ``{machine_id, limit, records, next_cursor, has_more}``
  envelope, where each record carries exactly the complete rotation fields
  (the six visible fields plus ``previous_rotation_id``/``content_hash``/
...
- a rotation with an unparseable ``created_at`` kept verbatim, sorted last,
...
def insert_rotation(
    ...
    *,
    ...
    if old_public_key is None:
        old_public_key = f"old-key-{n}"
    if new_public_key is None:
        new_public_key = f"new-key-{n}"
    if version is None:
        version = n + 1
    values = {
        "id": rid(n),
...
@pytest.mark.parametrize(
    "cursor",
    [
        # Well-shaped positions that name no stored rotation; the timestamp
...
def test_cursor_pointing_at_an_actual_rotation_is_accepted(client, machine_id):
...
def test_cursor_naming_another_machines_rotation_is_422(client, machine_id):
    other = client.post(
        "/machines",
        json={
    ...
    insert_rotation(client, other["id"], 1, T0)
...
def test_parameter_errors_take_priority_over_machine_lookup(client):
    ...
    # A well-shaped cursor names no rotation of a missing machine, so it is
...
def test_missing_machine_is_404_with_no_rotation_records(client):
...
def test_records_carry_exactly_the_complete_chain_fields(client, machine_id):
    stored = insert_rotation(
        ...
        old_public_key="old-7", new_public_key="new-7", version=8,
        previous_rotation_id=rid(3),
    ...
    assert record == {
        "id": rid(7),
        "machine_id": machine_id,
        "old_public_key": "old-7",
        "new_public_key": "new-7",
        "version": 8,
...
def test_only_the_path_machines_records_are_returned(client, machine_id):
    ...
    insert_rotation(client, machine_id, 1, T0)
    insert_rotation(client, other["id"], 2, T1)
...
def test_changes_pages_match_the_list_view_slice(client, machine_id):
...
def test_real_rotations_are_pageable(client, machine_id):
    for version in (1, 2, 3):
        rotated = client.post(
            f"/machines/{machine_id}/rotate-key",
            json={"public_key": f"key-{version + 1}", "expected_version": version},
        )
        assert rotated.status_code == 200

    records, _ = fetch_all_pages(client, machine_id, limit=2)
    assert [r["version"] for r in records] == [2, 3, 4]
    assert records[0]["old_public_key"] == "key-1"
    assert records[0]["new_public_key"] == "key-2"
    assert records[0]["previous_rotation_id"] is None
    assert records[1]["previous_rotation_id"] == records[0]["id"]
    assert records[2]["previous_rotation_id"] == records[1]["id"]
...
def test_new_earlier_inserts_do_not_revisit_returned_pages(client, machine_id):
    ...
    # Insert a rotation sorting before the cursor position and one between.
...
def test_body_is_compact_newline_terminated_json_with_fixed_order(
    ...
    insert_rotation(
        ...
        old_public_key="old-1", new_public_key="new-1", version=2,
    ...
    expected_item = {
        "id": rid(1),
        "machine_id": machine_id,
        "old_public_key": "old-1",
        "new_public_key": "new-1",
        "version": 2,
        "created_at": T1,
        "previous_rotation_id": None,
        "content_hash": HASH_A,
...
def test_utf8_content_is_round_tripped_byte_stably(client, machine_id):
    ...
    assert first == second
    assert "轮换".encode("utf-8") in first
    body = json.loads(first)
    assert body["records"][0]["new_public_key"] == "key-轮换-★"
