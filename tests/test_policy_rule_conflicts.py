...
stored with a ``relation`` marker (``invalid``/``unmatched``/``conflict``/
``overridden``), candidate formation (same action type, intersecting
resource patterns), conflict groups (equal priority, opposite effects),
override relations (smaller priority covers the larger), the existing
``*`` resource-pattern intersection semantics, exclusion of invalid rules
from matching, created-instant-then-id detail ordering and id-pair group
ordering, ``invalid_query``/405 handling before any rule is read, 500 with
no partial analysis, empty-database results, tolerance of damaged records,
...
def test_rule_details_carry_stored_fields_and_relation(client):
    # Disjoint patterns and distinct priorities keep both rules unmatched so
    # the detail shape is asserted without conflict/override annotations.
...
def test_details_ordered_by_created_instant_then_id(client):
    ...
    ids = [rule["id"] for rule in client.get(PATH).json()["rules"]]
    assert ids == [rid(1), rid(2), rid(10), rid(20), rid(21), rid(30)]


def test_damaged_created_at_sorts_last_but_is_kept_and_unmatched(client):
    insert_rule_row(client, rid(2), T1, resource_pattern="p2")
    insert_rule_row(client, rid(3), "not-a-timestamp", resource_pattern="p3")
    insert_rule_row(client, rid(1), T0, resource_pattern="p1")

    rules = client.get(PATH).json()["rules"]
    assert [rule["id"] for rule in rules] == [rid(1), rid(2), rid(3)]
    assert rules[-1]["created_at"] == "not-a-timestamp"
    assert rules[-1]["relation"] == "unmatched"
...
def test_conflict_member_is_marked_even_when_also_covered(client):
    # rid(2) and rid(3) conflict at priority 1; rid(1) covers both at 0. The
    # conflict group still exists and its members stay marked ``conflict``
    # (conflict is the stronger relation), while the covering rule is
    # unmatched and both covering pairs are still reported as overrides.
    insert_rule_row(client, rid(1), T0, priority=0, effect="allow",
                    resource_pattern="res/*")
    insert_rule_row(client, rid(2), T1, priority=1, effect="allow",
                    resource_pattern="res/a*")
    insert_rule_row(client, rid(3), T2, priority=1, effect="deny",
                    resource_pattern="res/a/b*")

    body = client.get(PATH).json()
    assert body["conflicts"] == [
        {
            "rule_ids": [rid(2), rid(3)],
    ...
    assert body["overrides"] == [
        {
            "covering_rule_id": rid(1),
            "covered_rule_id": rid(2),
            "intersection": "res/a*",
            "reason": "lower_priority_overrides",
        },
        {
            "covering_rule_id": rid(1),
            "covered_rule_id": rid(3),
            "intersection": "res/a/b*",
            "reason": "lower_priority_overrides",
        },
    ]
    relations = {rule["id"]: rule["relation"] for rule in body["rules"]}
    assert relations[rid(1)] == "unmatched"
    assert relations[rid(2)] == "conflict"
    assert relations[rid(3)] == "conflict"
...
def test_lower_priority_value_covers(client):
    ...
    relations = {rule["id"]: rule["relation"] for rule in body["rules"]}
    assert relations[rid(1)] == "unmatched"
    assert relations[rid(2)] == "overridden"


def test_override_applies_regardless_of_effect(client):
    # Same effect and different priorities still shadow: the smaller priority
    # value has decisive effect over the intersecting scope.
    ...
    assert body["overrides"] == [
        {
            "covering_rule_id": rid(2),
            "covered_rule_id": rid(1),
            "intersection": "res/*",
            "reason": "lower_priority_overrides",
        }
    ]
...
def test_overrides_are_sorted_by_covering_then_covered_id(client):
    insert_rule_row(client, rid(1), T0, priority=1, effect="allow")
    ...
    body = client.get(PATH).json()
    assert [
        (entry["covering_rule_id"], entry["covered_rule_id"])
    ...
    assert relations[rid(1)] == "unmatched"
    assert relations[rid(2)] == "overridden"
    assert relations[rid(3)] == "overridden"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # blob; the details still surface it in a deterministic textual form.
        ("action_type", b"read"),
...
def test_invalid_rule_never_covers_a_larger_priority_rule(client):
    # An illegal deny at nominal priority 1 must not cover the valid allow at
    # priority 5: no override and the allow stays unmatched.
    insert_rule_row(client, rid(1), T0, priority=1, effect="maybe")
    insert_rule_row(client, rid(2), T1, priority=5, effect="allow")
    ...
    assert relations[rid(2)] == "unmatched"
...
def test_analysis_does_not_change_listing_or_authorization(client):
