"""Tests for the read-only global policy-rule conflict/override analysis.
...
stored with a ``valid`` marker, the conflict rule (same action type,
intersecting resource patterns, equal priority, opposite effects), the
override rule (same scope, different priorities, the smaller value
overriding), the existing ``*`` resource-pattern intersection semantics,
exclusion of invalid rules from matching, id-ascending stable ordering,
``invalid_query``/405 handling before any rule is read, empty-database
results, tolerance of damaged records, strict read-only byte stability, and
...
def test_rule_details_carry_stored_fields_and_valid_flag(client):
    ...
    # Details are ordered by rule id ascending, not by creation time.
...
def test_invalid_rules_never_conflict_or_override(client, field, value):
    ...
    body = client.get(PATH).json()
    assert [rule["valid"] for rule in body["rules"]] == [True, False]
    assert body["conflicts"] == []
    assert body["overrides"] == []
...
def test_invalid_rules_still_appear_in_details(client):
    ...
    assert body["rules"][0]["valid"] is False
